...
class house_hunter:
    ...
    def push_dict_to_shelf(link : str,dict : dict):
        # file to be used
        shelf = shelve.open("full_data.shlf")
        # serializing
        shelf[dict[link]]=dict
        shelf.close() # you must close the shelve file!!!

    def check_link_shelf(link : str):
        shelf = shelve.open("full_data.shlf") # the same filename that you used before, please
        try:
            my_dict = shelf[link]
        except:
    ...
    def read(self, gmaps_true : bool, transport_mode : str, list_of_links : list):
        ...
        single_read(self,gmaps_true,transport_mode)
